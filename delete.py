    ...
    for row in data_rows:
        ...
        if datetime.now() > deletion_date:
            ...
            try:
                ...
                logging.info("Deleted directory at path: %s", del_VM)
            except OSError as e:
                print(f"Error deleting directory at path: {del_VM}: {e}")
                print(f"Deleted directory at path: {del_VM}")
                logging.error("Error deleting directory at path: %s: %s", del_VM, e)
                logging.info("Deleted directory at path: %s", del_VM)
...
def oldest_VM_path(conn):
    ...
    oldest_vm_path = os.path.join("/home/c0a21137/", vm_path)
